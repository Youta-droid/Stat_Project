...
#' creates hovering message windows for the intarcrive prot by protly
#' returns the informations about countryname, year, the value of the regarding parameter
#' and happiness score
#' @param label The name of the parameter in question (gdp, corruption, etc.)
#' @return A caracter string. 
...
# the valiables for the column names are wrapped in {{}}. The actual columns are not. 
#' the function creates a scatter plot with a linear regression WITHOUT captions. 
#' @param parameter_name the name of the indicator you want to display in the window in side of create_hover
#' @param use_loess you can choose if you use a loess function
#' @return the plot in the minimal style WITHOUT any captions
ggplot_scatter_linear <- function(data, x, y, parameter_name, use_loess = FALSE){
  p <- ggplot(
    data = data,
    aes(x = {{ x }},
    y = {{ y }},
...
p_gdp <- ggplot_scatter_linear(happiness_complete, gdp, happiness, "GDP contribution", use_loess = TRUE) + labs(
...
p_social <- ggplot_scatter_linear(happiness_complete, social_support, happiness, "Social support", use_loess = FALSE)  +
  labs(
