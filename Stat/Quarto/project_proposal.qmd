...
```{r ida-missingness, warning=FALSE, message=FALSE}
# core variables used throughout the analysis
core_vars <- c("happiness", "gdp", "social_support", "health",
               "freedom", "generosity", "corruption")

# per-variable missingness on the cleaned (pre-filter) data
missing_summary <- happiness_clean %>%
  summarise(across(all_of(core_vars), ~ sum(is.na(.)))) %>%
  pivot_longer(everything(), names_to = "variable", values_to = "n_missing") %>%
  mutate(pct_missing = round(100 * n_missing / nrow(happiness_clean), 1)) %>%
  arrange(desc(n_missing))

knitr::kable(
  missing_summary,
  col.names = c("Variable", "Missing (n)", "Missing (%)"),
  caption = "Missing values per core variable (before filtering)"
)
```

```{r ida-rowdrop, warning=FALSE, message=FALSE}
# how many rows survive the complete-case filter
n_total    <- nrow(happiness_clean)
n_complete <- nrow(happiness_complete)
n_dropped  <- n_total - n_complete

knitr::kable(
  tibble(
    metric = c("Total rows (cleaned)", "Complete cases kept", "Rows dropped", "% dropped"),
    value  = c(n_total, n_complete, n_dropped, round(100 * n_dropped / n_total, 1))
  ),
  col.names = c("Metric", "Value"),
  caption = "Effect of the complete-case filter"
)
```

**Interpretation:**\
Of the `r n_total` rows in the cleaned data, `r n_complete` are complete across all seven core variables and `r n_dropped` (`r round(100 * n_dropped / n_total, 1)`%) are removed by the complete-case filter. The missingness is not spread evenly: `corruption` and `generosity` account for most of the missing values, while the target variable `happiness` and `gdp` are almost fully observed. Because the share of dropped rows is small and concentrated in a few secondary predictors rather than in the target, complete-case analysis is a reasonable and unbiased-enough strategy for this project.
...
```{r ida-summary, warning=FALSE, message=FALSE}
# summary statistics for every core variable
summary_stats <- happiness_complete %>%
...
knitr::kable(
  ...
  caption = "Summary statistics for each core variable (complete-case data)"
)
```

```{r ida-histograms, warning=FALSE, message=FALSE}
# faceted histograms to compare the shape of each variable
happiness_complete %>%
  select(all_of(core_vars)) %>%
  pivot_longer(everything(), names_to = "variable", values_to = "value") %>%
  ggplot(aes(x = value)) +
  ...
  labs(
    ...
    subtitle = "Complete-case data used in the analysis",
    x = "Value",
    y = "Count"
  ) +
  theme_minimal()
